...
extract_list_path = os.path.join(os.path.dirname(__file__),extractconfig.EXTRACT_LIST_FILE)
extract_list = pd.read_excel(extract_list_path, engine='openpyxl')
extract_list = extract_list['Indicator Name'].tolist()

#Create Pivot for each file in ExtractList.xlsx
for e in extract_list:
    if (not(e.strip() in raw_data['Indicator Name'].values)):
